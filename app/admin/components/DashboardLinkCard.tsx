...
const DashboardLinkCard: FC<DashboardLinkCardProps> = ({ href, title, description }) => (
  <Link
    href={href}
    className="bg-[#67AEFF]/15 rounded-xl px-6 py-10 shadow flex flex-col items-center hover:bg-[#67AEFF]/20 transition-colors"
  >
    {/* {icon && <div className="mb-2 text-3xl text-[#687FE5]">{icon}</div>} */}
    <span className="text-2xl font-bold text-[#67AEFF]">{title}</span>
